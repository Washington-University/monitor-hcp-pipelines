...
for subject in ${subjects} ; do
	if [[ ${subject} != \#* ]]; then
		./CheckForUpdateFunctionalPreprocPackagesCompletion.sh --project=${project} --archive-root="/data/hcpdb/archive/${project}/arc001" --subject=${subject} --output-dir="/data/hcpdb/packages/PostMsmAll"
	fi
done
